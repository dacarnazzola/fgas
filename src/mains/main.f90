...
use, non_intrinsic :: util, only: covariance=>cov, cholesky_decomposition=>chol, apply_constraints=>constraints_reflective_boundary
...
public :: stdout, ik, rk
public :: twopi
public :: covariance, cholesky_decomposition, apply_constraints, random_uniform
...
contains
    ...
    impure subroutine solve_rastrigin(problem_dimension, population_size, maximum_generations)
        ...
        real(rk) :: fitness(population_size), cov(problem_dimension,problem_dimension), chol(problem_dimension,problem_dimension), &
                    ...
                    regularization_vector(problem_dimension), ft, ft_new
        ...
        integer(ik) :: selected_pairs_ii(2,population_size-1), generation, elite_ii, failed_gen, i
        ...
        ! start mutation scale at 1.0, it will vary on [1.0,4.0] depending on generational fitness
        mutation_scale = 2.0_rk
        ...
        do generation=1,maximum_generations
            ...
            call perform_mutation(new_population(:,1:population_size-1), mutation_rate, chol, mutation_scale)
            call apply_constraints(new_population, domain_lb, domain_ub)
            ...
            if (failed_gen < maximum_generations/10) then
                ft = ft_new
                ...
                do concurrent (i=1_ik:population_size-1)
                    new_population(:,i) = new_population(:,population_size)
                end do
                call perform_mutation(new_population(:,1:population_size-1), 1.0_rk, chol, 1.0_rk)
                call apply_constraints(new_population, domain_lb, domain_ub)
